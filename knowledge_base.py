...
# processing university data into graph
with open("university_data.csv") as data:
    file = csv.reader(data, delimiter=',')
    for row in file:
        university = URIRef(exdata + row[0].replace(" ", "_")) # define university URI using first column
        link = URIRef(exdata + row[1]) # define link URI to university's entry in dbpedia as second column

        g.add( (university, RDF.type, ex.University) )
        g.add( (university, FOAF.name, Literal(row[0])) )
        g.add( (university, RDFS.seeAlso, link) )

# processing course data into graph

# processing topic data into graph

# processing student data into graph
